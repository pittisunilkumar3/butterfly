<template>
    <a-layout>
        ...
        <a-layout style="margin-left: 240px">
            <AdminPageHeader>
                <template #header>
                    <a-page-header :title="$t(`menu.analytics`)" class="p-0" />
                </template>
                <template #breadcrumb>
                    ...
                    <a-row v-else :gutter="[15, 15]" class="mt-30 mb-20">
                        ...
                        >
                            <a-card 
                                ...
                                hoverable 
                                @click="() => $router.push({ name: 'admin.analyze.campaign.leads', params: { id: allCampaign.xid }})"
...
export default {
    ...
    setup() {
        ...
        const { t } = useI18n();
        const campaignsUrl =
            "call-managers?fields=id,xid,campaign_id,name,reference_prefix,allow_reference_prefix,remaining_leads,total_leads,campaignUsers{id,xid,user_id,x_user_id,campaign_id,x_campaign_id},campaignUsers:user{id,xid,name,profile_image,profile_image_url},form_id,x_form_id,form{id,xid,name,form_fields},detail_fields,last_action_by,x_last_action_by,lastActioner{id,xid,name},completed_by,x_completed_by,completedBy{id,xid,name},started_on,completed_on,upcoming_lead_action";
        ...
        const fetchCampaigns = () => {
            ...
            Promise.all([campaignsPromise]).then(([campaignsResponse]) => {
                // Prioritize campaign_id, then id, then xid
                const processedCampaigns = campaignsResponse.data.map(campaign => ({
                    ...campaign,
                    campaign_id: campaign.campaign_id || campaign.id || campaign.xid,
                    hasValidCampaignId: !!(campaign.campaign_id || campaign.id || campaign.xid)
                }));

                // Filter out campaigns without a valid campaign ID
                const validCampaigns = processedCampaigns.filter(campaign => campaign.hasValidCampaignId);

                if (validCampaigns.length === 0) {
                    notification.warning({
                        message: t('campaign.no_valid_campaigns'),
                        description: t('campaign.campaigns_id_missing')
                    });
                }

                allCampaigns.value = validCampaigns;
            }).catch(error => {
                console.error('Error fetching campaigns:', error);
                notification.error({
                    message: t('common.error'),
                    description: t('campaign.fetch_error')
                });
            });
        };
